...
foreach ($resourceGroup in $resourceGroups) {
  # Get all resources of type microsoft.dbformysql/flexibleservers in each resource group
  $resources = Get-AzResource -ResourceGroupName $resourceGroup.ResourceGroupName | Where-Object {$_.ResourceType -eq "microsoft.dbformysql/flexibleservers"}
...
$RGTYScope = $RGAlert.properties.condition.allOf.anyof | Where-Object { $_.field -eq "resourceType" -and $_.equals -eq "microsoft.dbformysql/flexibleservers" } 
...
if ($null -eq $RGTYScope -and $flexibleservers.count -gt 0) {
  ...
  $newResourceTypev1 = "microsoft.dbformysql/flexibleservers"
