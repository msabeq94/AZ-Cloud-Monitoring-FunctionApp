...
foreach ($resourceGroup in $resourceGroups) {
  # Get all resources of type microsoft.network/loadbalancers in each resource group
  $resources = Get-AzResource -ResourceGroupName $resourceGroup.ResourceGroupName | Where-Object {$_.ResourceType -eq "microsoft.network/loadbalancers"}
  $loadbalancers += $resources  # Add the found storage accounts to the array
}

$RGAlert= Invoke-RestMethod -Uri $RGhealthURI -Method get -Headers $header 
$RGTYScope = $RGAlert.properties.condition.allOf.anyof | Where-Object { $_.field -eq "resourceType" -and $_.equals -eq "microsoft.network/loadbalancers" } 


if ($null -eq $RGTYScope -and $loadbalancers.count -gt 0) {

  $newResourceType = @{
    "field" = "resourceType"
    "equals" = "microsoft.network/loadbalancers"
  }
  $newResourceTypev1 = "microsoft.network/loadbalancers"
...
"@
      ...
      start-sleep -s 5
        ...
        $functionURI ="https://management.azure.com/subscriptions/f5980816-b478-413b-ae0b-5fb6d820a88f/resourceGroups/vf-core-uk-resources-rg/providers/Microsoft.Web/sites/VF-Core-Function/functions/RGTY-Health-Alert-network-loadbalancers?api-version=2015-08-01"
        Invoke-RestMethod -Uri $functionURI -Method Delete -Headers $header 
      } else {
        write-output "No network-loadbalancers resources found"
      } 
